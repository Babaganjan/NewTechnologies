...
//                     href={item.href}
//                     variant="menu"
//                     className={`nav__link ${item.title === activeNavItem ? 'hovered' : ''}`}
      ...
      <div className="container header__container flex">
        <div className="header__wrapper flex">
          <nav className="header__nav nav hidden">
            <ul className="nav__list flex">
              {navItems.map((item) => {
                // Класс hovered ТОЛЬКО для активного элемента
                ...
                return (
                  ...
                  >
                    <Button
                      href={item.href}
                      variant="menu"
                      className={`nav__link ${isHovered ? 'hovered' : ''}`} // Только для этого item
                      iconSmall={item.hasIcon}
                      onClick={onCloseModal}
                    >
                      {item.title}
